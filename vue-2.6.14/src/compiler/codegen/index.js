...
export function generate(
   ...
   * data 为节点上的属性组成的 JSON字符串，比如：'{key:xx, ref:xx,...}'
   * children 为所有子节点的字符串格式的代码组成的字符串数组，格式：
   *   `['_c(tag, data, children)',...],normalizationType`
   *   最后的nornalization 是 _c的是四个参数
   *   表示节点的规范化类型，不是重点，不需要关注
   * 当然 code 并不一定就是 _c, 也有可能是其他的。比如整个组件都是静态的，则结果就为：_m(0)
   */
  const code = ast
    ? ast.tag === "script"
      ? "null"
...
export function genElement(el: ASTElement, state: CodegenState): string {
  ...
  if (el.staticRoot && !el.staticProcessed) {
     ...
     * 1.当前节点存在 v-if 指令，得到一个三元运算符， condition ? render1 : render2
    ...
    /**
     * 处理 v-if 指令的节点，最终得到一个三元表达式，condition ? render1 : render2
     */
    ...
    /**
     * 生成插槽的渲染函数，得到
     * _(slotName, children, attrs, bind)
     */
    ...
    if (el.component) {
      ...
      if (!el.plain || (el.pre && state.maybeComponent(el))) {
        // 非普通元素或者带有 v-pre 指令的组件走这里。处理节点的所有属性，返回一个JSON字符串
        // 比如：'{key:xx, ref:xx, ...}'
      ...
      // '['_c(tag, data, children)', ...],normalizationType'
      // 最后的 normalizationType 表示节点的规范化类型，不是重点，不需要关注
...
function genStatic(el: ASTElement, state: CodegenState): string {
  ...
  // idx = 当前静态节点的渲染函数在 staticRenderFns 数组中的小标
...
export function genIf(
  ...
  // 标记当前节点的 v-if 指令是否已经被处理过了，避免重复处理
  el.ifProcessed = true; // avoid recursion
  // 得到一个三元表达式，condition ? render1 : render2
...
function genIfConditions(
  ...
  if (condition.exp) {
    ...
    // 如果条件不成立，则通过递归的方式找 conditions 数组中的下一个元素，
...
export function genFor(
  ...
  // 得到 `_l(exp, functin(alias, iterator1, iterator2){return _c(tag, data, children)})`
  return (
    `${altHelper || "_l"}((${exp}),` +
...
export function genData(el: ASTElement, state: CodegenState): string {
   ...
   * 执行指令编译方法，比如 web 平台的 v-text, v-html, v-model, 然后在 el 对象上添加对应的属性
   * 比如： v-text: el.textContent = _s(value, dir)
   *       v-html: el.innerHtml = _s(value, dir)
   * 当指令运行时还有任务时，比如：v-model, 则返回 directives: [{name, rawName, value, arg, modifiers}, ...]
   */
...
export function genChildren(
  ...
  if (children.length) {
    ...
    // 优化，只有一个子节点 && 子节点上有 v-for 指令 && 子节点的标签不为 template 或者 slot
    ...
    // 获取节点规范化类型，返回一个 number0, 1, 2 不是重点，不重要
    const normalizationType = checkSkip
