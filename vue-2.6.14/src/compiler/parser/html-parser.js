...
export function parseHTML(html, options) {
  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      // 每处理完一种情况，就会截断（continue）循环，并且重置 html 字符串，将处理过的标签截掉。下一次循环处理剩余的 html 字符串模板
      if (textEnd === 0) {
        // Comment:
        // 处理注释标签 <!-- xxx -->
        ...
        if (startTagMatch) {
          // 进一步处理上一步得到的结果，并最后调用 options.start 方法
      ...
      let text, rest, next;
      if (textEnd >= 0) {
        // 能走到这儿，说明虽然在html中匹配到了 <xxx，但是这不属于上述几种情况
        // 它就只是一个普通的一段文本，例如：我是文本<
        // 于是从 html 中找到下一个 < , 直到 <xx 是上述几种情况的标签，则结束
        // 在这整个过程中一直在调整 textEnd 的值，作为 html 中下一个有效标签的开始位置

        // 截取 html 模板字符串中 testEnd 之后的内容 rest = <xxx
        ...
        while (
          ...
          // 如果没有找到 <,则直接结束循环
          if (next < 0) break;
          // 走到这说明在后续的字符串中找到了 < ,索引位置为 textEnd
          textEnd += next;
          // 截取 html 字符串模板 textEnd 之后的内容赋值给rest,继续判断之后的字符串是否存在标签
          rest = html.slice(textEnd);
        }
      ...
      // 如果 textEnd < 0, 说明 html 中就没找到 <,那就说明 html 就是一段文本
      if (textEnd < 0) {
        text = html;
      }

      // 将文本内容从 html 模板字符串中截取掉
      if (text) {
        advance(text.length);
      }
      ...
      // 匹配并处理开始标签和结束标签之间的所有文本，比如：<script>xxx</script>
    ...
    // 到这里就处理结束了，如果 stack 数组中还有内容，则说明有标签没有被闭合，给出提示信息
    if (html === last) {
   ...
   * index 为 html 在原始的模板字符串中的开始索引，也就是下一次处理的字符串的开始位置
   * @param {*} n 索引
   */
  function advance(n) {
    index += n;
