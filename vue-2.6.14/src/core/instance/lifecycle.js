...
export function lifecycleMixin(Vue: Class<Component>) {
  ...
  Vue.prototype._update = function (vnode: VNode, hydrating?: boolean) {
    ...
    if (!prevVnode) {
      // 旧`VNode`不存在，表示首次渲染，即初始化页面走这里（initial render）
      vm.$el = vm.__patch__(vm.$el, vnode, hydrating, false /* removeOnly */);
    } else {
      // 响应式数据更新时，即更新页面时走这里 (updates)
...
export function mountComponent(
  ...
  if (process.env.NODE_ENV !== "production" && config.performance && mark) {
    ...
    updateComponent = () => {
      // 执行 vm._render函数，得到虚拟DOM，并将`vnode`传递给_update函数。接下来就到`patch`阶段了。
...
export function callHook(vm: Component, hook: string) {
  ...
  pushTarget();
  // 从实例配置对象中获取指定钩子函数，比如：mounted （handlers是个数组）
  ...
  if (vm._hasHookEvent) {
    vm.$emit("hook:" + hook); //vm.$emit('hook:mounted')
  }
  // 关闭依赖收集
  popTarget();
}
